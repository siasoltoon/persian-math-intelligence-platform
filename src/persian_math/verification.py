...
def verify_expression_result(expression: sp.Expr, claimed: Any) -> VerificationResult:
    try:
        ...
        if sp.simplify(expected - claimed) == 0:
            return VerificationResult(True, ConfidenceLevel.HIGH,
                                      ("symbolic_equivalence", "independent_simplification"), (expected,))
        if _numeric_equal(expected, claimed):
            return VerificationResult(True, ConfidenceLevel.MEDIUM,
                                      ("numerical_equivalence", "independent_simplification"), (expected,))
...
def verify_equation_solution(lhs: sp.Expr, rhs: sp.Expr, symbol: sp.Symbol, candidate: Any) -> VerificationResult:
    try:
        if not _domain_safe(lhs - rhs, symbol, candidate):
            return VerificationResult(False, ConfidenceLevel.HIGH, ("domain_invalid",), (candidate,))
        residual = sp.simplify((lhs - rhs).subs(symbol, candidate))
        if residual == 0:
            return VerificationResult(True, ConfidenceLevel.HIGH,
                                      ("direct_substitution", "zero_residual", "domain_checked"), (candidate,))
        if _numeric_equal(residual, 0):
            return VerificationResult(True, ConfidenceLevel.MEDIUM,
                                      ("numerical_residual_check", "domain_checked"), (candidate,))
...
def verify_solution_set(lhs: sp.Expr, rhs: sp.Expr, symbol: sp.Symbol, candidates: tuple[Any, ...]) -> VerificationResult:
...
def compare_independent_methods(expression: sp.Expr, claimed: Any, alternate: Any) -> VerificationResult:
    try:
    ...
    if equivalent:
        return VerificationResult(True, ConfidenceLevel.HIGH,
                                  ("independent_method_agreement",), (claimed, alternate))
    return VerificationResult(False, ConfidenceLevel.HIGH,
                              ("independent_method_disagreement",), (claimed, alternate))
