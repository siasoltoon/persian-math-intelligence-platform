...
_LOCALS = {"pi": sp.pi, "e": sp.E, "sqrt": sp.sqrt, "sin": sp.sin, "cos": sp.cos, "tan": sp.tan, "cot": sp.cot, "sec": sp.sec, "csc": sp.csc, "asin": sp.asin, "acos": sp.acos, "atan": sp.atan, "log": sp.log, "ln": sp.log, "exp": sp.exp, "abs": sp.Abs}
_SAFE_GLOBALS = {"Integer": sp.Integer, "Float": sp.Float, "Rational": sp.Rational, "Symbol": sp.Symbol, "Add": sp.Add, "Mul": sp.Mul, "Pow": sp.Pow, "Function": sp.Function, "pi": sp.pi, "E": sp.E}
...
def normalize_math_text(text: str) -> str:
    ...
    value = re.sub(r"([A-Za-z0-9_)]+)([⁰¹²³⁴⁵⁶⁷⁸⁹⁻⁺]+)", lambda m: m.group(1) + "**" + m.group(2).translate(SUPERSCRIPT_DIGITS), value)
    value = re.sub(r"([A-Za-z])([₀₁₂₃₄₅₆₇₈₉]+)", lambda m: m.group(1) + "_" + m.group(2).translate(_SUBSCRIPT_DIGITS), value)
    replacements = {"×":"*", "⋅":"*", "·":"*", "÷":"/", "−":"-", "–":"-", "—":"-", "√":"sqrt", "π":"pi", "∞":"oo", "∑":"sum", "∏":"prod", "∫":"integral", "≤":"<=", "≥":">=", "≠":"!=", "（":"(", "）":")", "［":"[", "］":"]", "｛":"{", "｝":"}"}
...
def _parse(text: str) -> sp.Expr:
    try:
        return parse_expr(text, local_dict=_LOCALS, global_dict=_SAFE_GLOBALS, transformations=_TRANSFORMATIONS, evaluate=True)
...
def parse_matrix(text: str) -> sp.MatrixBase:
    ...
    rows = [r.strip() for r in re.split(r";|\]\s*,\s*\[", inner.strip("[]")) if r.strip()]
    try:
        matrix_rows = [[_parse(item.strip()) for item in re.split(r",|\s+", row.strip(" []")) if item.strip()] for row in rows]
