...
def explain_solution(value: Any, *, method: str, level: EducationalLevel = EducationalLevel.HIGH_SCHOOL, verification: str | None = None, original: Any | None = None) -> Explanation:
    labels = {
        EducationalLevel.ELEMENTARY:"ساده و مرحله‌به‌مرحله", EducationalLevel.MIDDLE:"ساده و آموزشی",
        EducationalLevel.HIGH_SCHOOL:"آموزشی و استاندارد", EducationalLevel.ENTRANCE_EXAM:"آزمونی و نکته‌محور",
        EducationalLevel.UNIVERSITY:"دقیق و دانشگاهی", EducationalLevel.ADVANCED:"فنی و تحلیلی",
    ...
    steps.append(SolutionStep(descriptions.get(method, f"روش تخصصی «{method}» را اعمال می‌کنیم."), value))
    steps.append(SolutionStep(f"سطح توضیح: {labels[level]}."))
    return Explanation("حل مسئله", tuple(steps), f"پاسخ نهایی: {_format(value)}", verification, ("نتیجه فقط پس از Verification مستقل، تأییدشده اعلام می‌شود.",), ("بی‌دقتی در علامت، پرانتز و دامنه از خطاهای رایج‌اند.",))
