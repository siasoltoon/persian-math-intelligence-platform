...
def solve_equation(lhs: sp.Expr, rhs: sp.Expr, symbol: sp.Symbol) -> SolverResult:
    try:
        return _ok(tuple(sp.solve(sp.Eq(lhs, rhs), symbol, dict=False)), "symbolic_equation", symbol=str(symbol))
    except (TypeError, ValueError, NotImplementedError) as exc:
        return _fail("symbolic_equation", exc)

def solve_inequality(lhs: sp.Expr, rhs_or_operator: str | sp.Expr, rhs_or_symbol: sp.Expr | sp.Symbol, symbol: sp.Symbol | None = None) -> SolverResult:
    try:
        if symbol is None:
            operator = rhs_or_operator
            rhs = rhs_or_symbol
            raise ValueError("symbol is required")
        relation = {"<": sp.Lt, "<=": sp.Le, ">": sp.Gt, ">=": sp.Ge}[str(rhs_or_operator)](lhs, rhs_or_symbol)
...
def solve_polynomial(expression: sp.Expr, symbol: sp.Symbol) -> SolverResult:
    try:
        ...
        return _ok(tuple(sp.solve(poly, symbol)), "polynomial", symbol=str(symbol), degree=poly.degree())
    except (TypeError, ValueError, NotImplementedError) as exc:
...
def solve_trigonometric(expression: sp.Expr, symbol: sp.Symbol) -> SolverResult:
    try:
        return _ok(tuple(sp.solveset(expression, symbol, domain=sp.S.Reals)), "trigonometric", symbol=str(symbol))
    except (TypeError, ValueError, NotImplementedError) as exc:
...
def solve_complex(expression: sp.Expr, symbol: sp.Symbol) -> SolverResult:
    try:
        return _ok(tuple(sp.solveset(expression, symbol, domain=sp.S.Complexes)), "complex_solution", symbol=str(symbol))
    except (TypeError, ValueError, NotImplementedError) as exc:
...
def optimize(expression: sp.Expr, symbol: sp.Symbol) -> SolverResult:
    try:
        ...
        return _ok({"critical_points": critical, "second_derivative": sp.diff(expression, symbol, 2)}, "symbolic_optimization", symbol=str(symbol))
    except (TypeError, ValueError, NotImplementedError) as exc:
...
def solve(expression_text: str, *, symbol_name: str = "x") -> SolverResult:
    ...
    try:
        if any(op in text for op in ("<", ">", "≤", "≥")):
            lhs, op, rhs = __import__("persian_math.canonical", fromlist=["parse_inequality"]).parse_inequality(text)
        ...
        if expression.free_symbols:
            ...
            if sp.Poly(expression, symbol).is_univariate and sp.Poly(expression, symbol).degree() >= 2:
