...
def run_benchmark(cases: tuple[BenchmarkCase, ...], runner: Callable[[str], str]) -> tuple[BenchmarkResult, ...]:
    results = []
    for case in cases:
        try:
            actual = runner(case.input_text)
            results.append(BenchmarkResult(case.case_id, actual == case.expected, actual, case.expected))
        except Exception:
