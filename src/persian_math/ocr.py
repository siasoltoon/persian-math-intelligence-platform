...
class TesseractOcrBackend:
    ...
    def _recognize_variant(self, image: Image.Image, psm: int) -> OcrResult:
        data = pytesseract.image_to_data(
            image, lang=self.language, config=f"--psm {psm}", output_type=pytesseract.Output.DICT
        )
        regions = []
        confidences = []
        ...
        return OcrResult(" ".join(r.text for r in regions), sum(confidences) / len(confidences) if confidences else 0.0, tuple(regions))
    ...
    def recognize(self, image: bytes, metadata: ImageMetadata) -> OcrResult:
        ...
        # Prefer confidence, then useful character yield. A consensus layer in the
        # application must still reject low-confidence math instead of guessing.
