...
def process(text: str, *, symbol_name: str = "x") -> EngineResult:
    ...
    try:
        if representation.kind == "system":
            ...
            symbols = tuple(
                sorted(
                    set().union(*(e.free_symbols for pair in equations for e in pair)),
                    key=str,
                )
            )
            ...
            for item in solver_result.value:
                if not isinstance(item, dict) or not all(
                    sp.simplify((lhs - rhs).subs(item)) == 0 for lhs, rhs in equations
                ):
                    verified = False
            verification = VerificationResult(
                verified,
                ConfidenceLevel.HIGH if verified else ConfidenceLevel.LOW,
                ("system_substitution", "independent_solver"),
                tuple(solver_result.value),
            )
        ...
        if representation.kind == "expression":
            ...
            candidates = (
                tuple(solver_result.value)
                if isinstance(solver_result.value, (tuple, list))
                else ()
            )
            ...
            relation = {"<": sp.Lt, "<=": sp.Le, ">": sp.Gt, ">=": sp.Ge}[operator](
                lhs, rhs
            )
            equivalent = sp.simplify(
                sp.Equivalent(relation, sp.And(solver_result.value, sp.S.true))
            )
            verified = equivalent is sp.true
            verification = VerificationResult(
                verified,
                ConfidenceLevel.HIGH if verified else ConfidenceLevel.LOW,
                ("inequality_equivalence",),
                (solver_result.value,),
            )
        return EngineResult(problem, solver_result, verification)
        ...
        verification = VerificationResult(
            False,
            ConfidenceLevel.LOW,
            ("engine_parse_or_verification_failure",),
        )
        return EngineResult(problem, failed, verification)
