...
def hint(session: TutorSession) -> TutorResponse:
    ...
    return TutorResponse(
        TutorAction.HINT,
        step.hint_fa or "داده‌های مسئله را جدا کن و روش مناسب را انتخاب کن.",
        None,
...
def check(session: TutorSession, answer: Any) -> tuple[TutorSession, TutorResponse]:
    ...
    if not correct:
        return session, TutorResponse(
            TutorAction.CHECK,
            "پاسخ با جواب مورد انتظار سازگار نیست. یک مرحله قبل را دوباره بررسی کن.",
            False,
            session.current_index,
        )
...
def reexplain(session: TutorSession) -> TutorResponse:
    step = current_step(session)
    style = {
        EducationalLevel.ELEMENTARY: "خیلی ساده و مرحله‌به‌مرحله",
        EducationalLevel.UNIVERSITY: "رسمی و دقیق",
    }.get(session.user.educational_level, "ساده و مرحله‌به‌مرحله")
    return TutorResponse(
        TutorAction.REEXPLAIN,
        f"این مرحله را {style} توضیح می‌دهیم: {step.prompt_fa}",
        None,
        session.current_index,
    )
...
def progressive_practice(user: UserProfile, topic: str, count: int = 3) -> tuple[str, ...]:
    ...
    return tuple(
        f"تمرین {i + 1}: یک مسئله {topic} در سطح {user.educational_level.value} حل کن."
